...
public class SingleShotGun : Gun
{
    ...
    void Awake()
    {
        //cooldown = PlayerPrefs.GetFloat("PISTOLCOOLDOWN");
    ...
    {   
        if(Time.time-lastShot<cooldown)
            return;
        lastShot = Time.time;
        StartCoroutine(cameraShake.Shake(.1f, .06f));
    ...
    void RPC_Shoot(Vector3 hitPosition, Vector3 hitNormal)
    {
        ...
        if(colliders.Length != 0)
        {
            ...
            if(bulletImpactObj.GetComponentInParent<playerGroundCheck>() || bulletImpactObj.GetComponentInParent<Rigidbody>())
